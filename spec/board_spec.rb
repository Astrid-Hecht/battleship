...
RSpec.describe Board do
  ...
  describe '#valid_placement?(ship, coordinates)' do
    ...
    it 'is not valid if the coordinates are not consecutive' do
      ...
      expect(board.valid_placement?(submarine, %w[A1 C1]))
    ...
    it 'is valid if the coordinates are valid' do
      ...
      expect(board.valid_placement?(destroyer, %w[A1 A2 A3 A4])).to eq(true)

      expect(board.valid_placement?(submarine, %w[A1 B1])).to eq(true)
      expect(board.valid_placement?(cruiser, %w[A1 B1 C1])).to eq(true)
      expect(board.valid_placement?(destroyer, %w[A1 B1 C1 D1])).to eq(true)
    end
  end
  ...
  describe '#place' do
    ...
    it "doesn't place at invalid coordinates" do
      expect(board.place(cruiser, %w[A1 A2])).to eq(nil)
      expect(board.place(cruiser, %w[A1 A2 A4])).to eq(nil)
      expect(board.place(cruiser, %w[A3 B2 C1])).to eq(nil)
    end
